...
// NewTinyBrainServer initializes the database, repositories, and services
// backing the MCP server. dbPath is the SQLite database file location.
...
func (t *TinyBrainServer) registerTools(s *mcp.Server) {
		...
		map[string]interface{}{
			...
			"properties": map[string]interface{}{
				...
				"intelligence_type": map[string]interface{}{
					"type":        "string",
					"description": "Intelligence gathering type: osint, humint, sigint, geoint, masint, techint, finint, cybint",
				},
				"classification": map[string]interface{}{
					"type":        "string",
					"description": "Classification level (e.g., unclassified, confidential, secret)",
				},
		...
		map[string]interface{}{
			...
			"properties": map[string]interface{}{
				...
				"category": map[string]interface{}{
					"type":        "string",
					"description": "Category: finding, vulnerability, exploit, payload, technique, tool, reference, context, hypothesis, evidence, recommendation, note, intelligence, osint, humint, sigint, geoint, masint, techint, finint, cybint, threat_actor, attack_campaign, ioc, ttp, pattern, correlation, target_analysis, infrastructure_mapping, vulnerability_assessment, malware_analysis, binary_analysis, vulnerability_research",
				},
				...
				"intelligence_type": map[string]interface{}{
					"type":        "string",
					"description": "Intelligence gathering type: osint, humint, sigint, geoint, masint, techint, finint, cybint (stored as intel:<type> tag)",
				},
				"threat_level": map[string]interface{}{
					"type":        "string",
					"description": "Threat level: low, medium, high, critical (stored as threat:<level> tag)",
				},
				"mitre_tactic": map[string]interface{}{
					"type":        "string",
					"description": "MITRE ATT&CK tactic ID, e.g. TA0001 (stored as mitre_tactic:<id> tag)",
				},
				"mitre_technique": map[string]interface{}{
					"type":        "string",
					"description": "MITRE ATT&CK technique ID, e.g. T1055 (stored as mitre_technique:<id> tag)",
				},
			},
	...
	s.AddTool("update_memory",
		...
		map[string]interface{}{
			...
			"properties": map[string]interface{}{
				...
				"tags": map[string]interface{}{
					"type":        "string",
					"description": "JSON array of tags (replaces existing tags)",
				},
				"source": map[string]interface{}{
					"type":        "string",
	...
	// Aliases matching the names used in the published documentation
	// (https://rainmana.github.io/tinybrain/api-reference/)
	s.AddToolAlias("create_memory", "store_memory",
		"Create a new memory entry (alias of store_memory)")
	s.AddToolAlias("search_memory", "search_memories",
		"Search stored memories (alias of search_memories)")
	s.AddToolAlias("get_related_entries", "get_related_memories",
		"Get entries related to a memory entry (alias of get_related_memories)")
}
...
func (t *TinyBrainServer) handleCreateSession(ctx context.Context, params map[string]interface{}) (interface{}, error) {
	...
	// Intelligence-gathering attributes are persisted in session metadata
...
func (t *TinyBrainServer) handleStoreMemory(ctx context.Context, params map[string]interface{}) (interface{}, error) {
	...
	// they remain searchable via tag search
	for param, prefix := range map[string]string{
...
// handleUpdateMemory updates a single memory entry by routing through the
// batch update path so validation and tag handling stay consistent.
func (t *TinyBrainServer) handleUpdateMemory(ctx context.Context, params map[string]interface{}) (interface{}, error) {
	...
	entries, err := t.repo.BatchUpdateMemoryEntries(ctx, []*models.UpdateMemoryEntryRequest{update})
	if err != nil {
		return nil, fmt.Errorf("failed to update memory: %v", err)
	}
	if len(entries) == 0 {
		return nil, fmt.Errorf("memory entry not found: %s", memoryID)
	}
	return entries[0], nil
}

// handleDeleteMemory deletes a single memory entry by ID.
func (t *TinyBrainServer) handleDeleteMemory(ctx context.Context, params map[string]interface{}) (interface{}, error) {
	...
	if err := t.repo.BatchDeleteMemoryEntries(ctx, []string{memoryID}); err != nil {
