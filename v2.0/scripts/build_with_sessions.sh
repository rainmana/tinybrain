...
echo -e "\033[1;33m=== Building TinyBrain v2.0 with Session Management ===\033[0m"
...
OUTPUT_NAME="tinybrain-v2-sessions"

# Build the server with session management
go build -o "$OUTPUT_NAME" ./cmd/server/minimal_v2.go

if [ $? -eq 0 ]; then
    echo -e "\033[0;32m✅ Build successful: $OUTPUT_NAME\033[0m"
    echo ""
    echo "To run the server with session management:"
    ...
    echo "  ✅ Session management with proper collections"
